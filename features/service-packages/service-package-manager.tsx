...
import { AlertTriangle, Archive, CheckCircle2, Copy, PackagePlus, Pencil, Power } from "lucide-react";
...
import { archiveServicePackageAction, saveServicePackageAction, setServicePackageActiveAction } from "@/lib/actions/service-packages";
...
type ServiceOfferingOption = { value: string; label: string };
...
export function ServicePackageManager({
  ...
  permissions: { create: boolean; edit: boolean; duplicate: boolean; deactivate: boolean };
}) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><h1 className="text-2xl font-semibold tracking-tight">Services &amp; Packages</h1><p className="mt-1 text-sm text-muted">A Service Package is a reusable bundle — it automatically applies the workflow, organizer, letter, checklist, pricing, reviewer, deadline, invoice, release, and portal defaults for an engagement type.</p></div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-4">
            ...
            {permissions.deactivate && (
              <Button
                ...
                disabled={pending}
                onClick={() => {
                  if (!confirm(`Archive "${item.name}"? It will no longer be assignable, but existing engagements keep their configuration.`)) return;
                  run(() => archiveServicePackageAction(item.id));
                }}
              >
    ...
    <PackageDialog state={editing} open={editing !== null} onClose={() => setEditing(null)} serviceOfferings={serviceOfferings} />
  </div>;
}
...
function PackageDialog({
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{isEdit ? "Edit service package" : state.mode === "copy" ? "Duplicate service package" : "Create service package"}</DialogTitle></DialogHeader>
    <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); setFormError(null); const fd = new FormData(event.currentTarget); startTransition(async () => { const result = await saveServicePackageAction({ id: isEdit ? source?.id : undefined, name: String(fd.get("name")), serviceOfferingId: String(fd.get("serviceOfferingId") ?? ""), engagementType: String(fd.get("engagementType")), returnType: String(fd.get("returnType") ?? ""), pricingMethod: String(fd.get("pricingMethod")) as "fixed", reviewerPolicy: String(fd.get("reviewerPolicy")) as "auto_ero", activationDefault: String(fd.get("activationDefault")) as "confirm_before_sending", requirePayment: fd.get("requirePayment") === "on", requireSignature: fd.get("requireSignature") === "on", requireReviewApproval: fd.get("requireReviewApproval") === "on", requireFilingAcceptance: fd.get("requireFilingAcceptance") === "on", createInvoiceOnActivation: fd.get("createInvoiceOnActivation") === "on", inviteOnSend: fd.get("inviteOnSend") === "on" }); if (result.error) setFormError(result.error); else { toast.success(result.success ?? "Saved."); onClose(); } }); }}>
      ...
      <div className="space-y-2"><Label htmlFor="serviceOfferingId">Service</Label><Select name="serviceOfferingId" defaultValue={source?.serviceOffering?.id ?? "none"}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Not linked to a service</SelectItem>{serviceOfferings.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted">What the firm sells (e.g. Tax Preparation) — distinct from the engagement/return type below.</p></div>
