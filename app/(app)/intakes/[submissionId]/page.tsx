...
import { REVIEW_ROLES, STAFF_ROLES } from "@/lib/types";
...
export default async function IntakeDetailPage({
  ...
  const detail = await getIntakeDetail(supabase, workspace.workspace.id, submissionId);
  if (!detail) notFound();

  const canReview = REVIEW_ROLES.includes(workspace.role);
  const canStaff = STAFF_ROLES.includes(workspace.role);

  const correlatedRequests = detail.client
    ? (await listDocumentRequestsForClient(supabase, workspace.workspace.id, detail.client.id)).filter(
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Intake detail" description={`Reviewer: ${detail.reviewer?.name ?? "Unassigned"}`} />
